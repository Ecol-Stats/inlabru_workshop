...
format:
  revealjs:
    footer: '<a href="https://ecol-stats.github.io/inlabru_workshop/St_Andrews/docs/" style="color:#ccc; padding:20px 30px; display:inline-block; margin:-20px -30px;">Home</a>'
    margin: 0
    logo:  INLA_logo.png
...
>
> `inlabru` has some additional features:
>
> \- accounting for observation processes
>
> \- non-linear dependence structures
...
-   There are no "*true but unknown*" parameters !
-   Every parameter is described by a probability distribution!
-   Evidence from the data is used to update the belief we had before we observed the data!
...
We construct a `linear predictor` of the mean (or a function of the mean) of our observations, which consists of the `model components`.
...
This means that **all dependencies** in the observations are accounted for by the linear predictor -- and its components
...
::: callout-note
We assume that, given the *linear predictor* $\eta$, the observations are independent of each other! Data dependence is expressed through the *components* in the linear predictor.
:::
...
**Note 1:** These are the *components* of our model! These explain the *dependence structure* in the data.
...
3.  A prior for each model component in the vector $\textbf{u}$ $$
    \mathbf{u} = \{\beta_0, \beta_1\}\sim\mathcal{N}(0,\mathbf{Q}^{-1})
    $$ **Note:** These always have a Gaussian prior and are used to explain the dependence structure in data!
...
4.  A prior for the non-Gaussian parameters $\theta$ – the properties of the model components (so-called hyperparameters) $$
    \theta = \sigma^2
    $$
...
\begin{aligned}
y_i|\eta_i, \sigma^2 & \sim \mathcal{N}(\eta_i,\sigma^2)\\
\eta_i & = \color{#FF6B6B}{\boxed{\beta_0}} + \color{#FF6B6B}{\boxed{\beta_1 x_i}}
\end{aligned}
$$
:::
...
\begin{aligned}
y_i|\eta_i, \sigma^2 & \sim \mathcal{N}(\eta_i,\sigma^2)\\
\eta_i & = \color{#FF6B6B}{\boxed{\beta_0 + \beta_1 x_i}}
\end{aligned}
$$
:::
...
Data can have several dependence structures: temporal, spatial,... new components!
...
-   Structured random effects (temporal effects, e.g. AR(1), spatial effects, e.g. regional effects...)
-   ...
...
One example with temporal datas: \
Rainfall over 1 mm in the Tokyo area for each calendar day in the years 1983-84
...
-   $f(\text{time}_t)$ is a RW2 model (this is just a smoother)

-   The smoothness is controlled by a hyperparameter $\tau_f$
...
## `inlabru` for time series {.smaller auto-animate="true"}
...
## `inlabru` for temporal data {.smaller auto-animate="true"}
...
## Example: spatial data – disease mapping

We observed larynx cancer mortality counts for males in 544 districts of Germany from 1986 to 1990 and want to construct a model.
...
-   **Stage 2** Latent field model $$
    ...
    -   A Gaussian random field $\omega(s)$ (we will discuss this later..)
-   **Stage 3** Hyperparameters
