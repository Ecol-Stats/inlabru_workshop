...
2. How do we choose the best between our models? - **Model Comparison**
...
- _inlabru_ provides some options available

. . . 

## Model Comparison/Validation in _inlabru_{.smaller}
...
- You can sample from the posterior using the _generate()_ function. These can be used to compute
  - log-scores
  - CRPS
  - ...
...
- Smaller values of the DIC indicate better trade-off between complexity and fit of the model.
...
\end{eqnarray}
$$
using _inlabru_ we estimate the _posterior distribution_ $\pi(\mathbf{u},\theta|\mathbf{y})$.
...
**NOTE:** In general this is NOT computed by _inlabru_ but needs to be approximated 
...
**BUT** _inlabru_ computes automatically two quantities that are useful for model check!
...
_inlabru_ does not provide such scores automatcally, but they can be computed using simulations from the posterior distribution.
...
1. Sample $y^{1k}_i\sim\pi(y_i|\mathbf{y})$ using _generate()_
...
- _inlabru_ provides some easy to compute alternatives
