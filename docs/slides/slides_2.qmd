...
library(mapview)
data(meuse)
meuse0 = st_as_sf(meuse, coords = c("x","y"), crs = "+init=epsg:28992") %>%
...
boundary = (fm_nonconvex_hull(meuse0))
...
cmp = ~ beta0(1) + beta1(dist, model = "linear") + space(geometry, model=meuse.spde )

lik = bru_obs(formula = log(zinc) ~ beta0 + beta1 + space,
              data = meuse0,
              family = "gaussian")
 fit = bru(cmp, lik)
...
$y(s)$ log. zinc concentration in location $s$
...
$$
y(s)|\eta(s)\sim\mathcal{N}(\eta(s), \sigma^2_y)
$$
+ **Stage 2** Latent field model
$$
\eta(s) = \beta_0 + \beta_1 x(s) + \omega(s)
$$
...
$$
  ...
  - A linear covariate $x(s)$ (distance from the river)
...
+ **Stage 3** Hyperparameters
  ...
  - Variance in the likelihood $\sigma_y$
  - Range and sd in teh Gaussian field $\sigma_{\omega}, \tau_{\omega}$
...
\begin{aligned}
y(s)|\eta(s) & \sim\mathcal{N}(\eta(s), \sigma^2_y)\\
\eta(s) & = \color{red}{\boxed{\beta_0}} + \color{red}{\boxed{\beta_1 x(s)}} + \color{red}{\boxed{\omega(s)}}
\end{aligned}
$$
:::
...
```{r}
#| echo: true
meuse0 %>% select(zinc, dist) %>% print(n = 3)
```
:::
:::
...
# define model predictor
eta = log(zinc) ~ beta0 + beta1 + space
...
\begin{aligned}
y(s)|\eta(s) & \sim\mathcal{N}(\eta(s), \sigma^2_y)\\
\color{red}{\boxed{\eta(s)}} & = \color{red}{\boxed{\beta_0 + \beta_1 x(s) + \omega(s)}}
\end{aligned}
$$
:::
...
$$
\begin{aligned}
\color{red}{\boxed{y(s)|\eta(s)}} & \color{red}{\boxed{\sim\mathcal{N}(\eta(s), \sigma^2_y)}}\\
\eta(s) & = \beta_0 + \beta_1 x(s) + \omega(s)
\end{aligned}
$$
:::
...
library(scico)
pixel = fm_pixels(mesh)
pred = predict(fit, pixel, ~ space)

p1 = ggplot() + geom_sf(data = pred, aes(color = mean)) + scale_color_scico(direction = -1)


p2 = ggplot() + geom_sf(data = pred, aes(color = sd)) + scale_color_scico(direction = -1)
