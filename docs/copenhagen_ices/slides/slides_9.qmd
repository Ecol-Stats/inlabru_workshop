...
We focus on **separable models** but some non-separable ones are available in `inlabru`^[paper]
...
mesh = fm_mesh_2d(boundary = st_simplify(border, dTolerance = 25),
...
fields = inla.qsample(k, Q)
for(i in 2:k)
  fields[,i] =    rho * fields[, i - 1] + sqrt(1 - rho^2) * fields[, i]

t = seq(from = -1, to = 1, length.out = 12)
v = scale(t^3+cos(t))/3
proj  = fm_evaluator(mesh, loc= coords) 
