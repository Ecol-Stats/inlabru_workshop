...
p1 = data.frame(x =x,
  ...
  geom_line(aes(x,u3, color = "k = 2")) + xlab("") + ylab("") + ggtitle("alpha = 1") + ylim(-1.5,1.5)
...
p2 = data.frame(x =x,
  ...
  geom_line(aes(x,u2, color = "k = 1")) +
  geom_line(aes(x,u3, color = "k = 2"))+ xlab("") + ylab("") + ggtitle("alpha = 2") + ylim(-1.5,1.5)
...
- The weights vector $\mathbf{w} = (w_1,\dots,w_N)$ is Gaussian with a **sparse** precision matrix $\longrightarrow$ Compotational convenience
...
lik_spde = bru_obs(formula = triceps ~.,
                   data = triceps)
