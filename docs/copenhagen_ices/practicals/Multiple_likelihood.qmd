...
In this example we are going to fit a model where one covariate acts in the same way for two different data
...
Fit three models using the components and likelihoods above:
  ...
  - A model that only considers both data sets
...
- $\omega_1(s)$ is Gaussian spatial effects that is common for both observations

- $\lambda$ is a weight for the spatial effect $\omega_1(s)$.


- $\omega_2(s)$ is Gaussian spatial effects specific for the second observations
...
Use the same priors for both ${\bf z}_1$ and ${\bf z}_2$ so we create only one `spde` object. 
...
## {{< bi pencil-square color=#c8793c >}} Task
 ...
 Use the function `generate()` to create 4 simulations from $\widetilde{\pi}(\omega_1(s)|{\bf y}_1,{\bf y}_2)$  and $\widetilde{\pi}(\omega_2(s)|{\bf y}_1,{\bf y}_2)$ 
