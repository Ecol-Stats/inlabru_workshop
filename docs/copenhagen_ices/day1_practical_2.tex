...
\section{Setting priors and model checking for Linear
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, leftrule=.75mm, rightrule=.15mm, opacityback=0, toprule=.15mm, coltitle=black, titlerule=0mm, left=2mm, colbacktitle=quarto-callout-note-color!10!white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, bottomtitle=1mm, toptitle=1mm, bottomrule=.15mm, colframe=quarto-callout-note-color-frame, opacitybacktitle=0.6, colback=white, breakable]
...
\subsubsection{\texorpdfstring{\textbf{Fitting the linear regression
model with
\texttt{inlabru}}}{Fitting the linear regression model with inlabru}}\label{fitting-the-linear-regression-model-with-inlabru}

Now we fit a simple linear regression model in \texttt{inlabru} by
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, leftrule=.75mm, rightrule=.15mm, opacityback=0, toprule=.15mm, coltitle=black, titlerule=0mm, left=2mm, colbacktitle=quarto-callout-warning-color!10!white, title={Task}, bottomtitle=1mm, toptitle=1mm, bottomrule=.15mm, colframe=quarto-callout-warning-color-frame, opacitybacktitle=0.6, colback=white, breakable]
...
\subsection{Visualizing the posterior
...
\section{Linear Mixed Model for fish weight-length
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cmp }\OtherTok{=}  \ErrorTok{\textasciitilde{}} \SpecialCharTok{{-}}\DecValTok{1} \SpecialCharTok{+}\NormalTok{ sex\_M }\SpecialCharTok{+} 
  \FunctionTok{beta\_0}\NormalTok{(}\DecValTok{1}\NormalTok{)  }\SpecialCharTok{+} 
  \FunctionTok{beta\_1}\NormalTok{(tl, }\AttributeTok{model =} \StringTok{"linear"}\NormalTok{) }\SpecialCharTok{+}  
  \FunctionTok{net\_eff}\NormalTok{(net\_no, }\AttributeTok{model =} \StringTok{"iid"}\NormalTok{)}
...
Time used:
    Pre = 0.918, Running = 0.188, Post = 0.0324, Total = 1.14 
Fixed effects:
          mean    sd 0.025quant 0.5quant 0.975quant    mode kld
sex_M   -1.106 0.218     -1.534   -1.106     -0.678  -1.106   0
beta_0 -15.816 0.871    -17.516  -15.819    -14.098 -15.819   0
...
Precision for net_eff                   2.150 1.326      0.563    1.837
                                        0.975quant  mode
Precision for the Gaussian observations      0.568 0.469
Precision for net_eff                        5.568 1.316
...
\texttt{inlabru} functions to achieve this (see supplementary note).

We use the \texttt{predict} function to draw samples from the
approximated joint posterior for the hyperparameters, then invert them
to get variances and lastly compute the mean, std. dev., quantiles, etc.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sampvars }\OtherTok{\textless{}{-}}  \FunctionTok{predict}\NormalTok{(fit,PygmyWFBC, }\SpecialCharTok{\textasciitilde{}}\NormalTok{ \{}
\NormalTok{   tau\_e }\OtherTok{\textless{}{-}}\NormalTok{ Precision\_for\_the\_Gaussian\_observations}
\NormalTok{   tau\_u }\OtherTok{\textless{}{-}}\NormalTok{ Precision\_for\_net\_eff}
   \FunctionTok{list}\NormalTok{(}\AttributeTok{sigma\_u =} \DecValTok{1}\SpecialCharTok{/}\NormalTok{tau\_u,}
        \AttributeTok{sigma\_e =} \DecValTok{1}\SpecialCharTok{/}\NormalTok{tau\_e)}
\NormalTok{   \},}
   \AttributeTok{n.samples =} \DecValTok{1000}
\NormalTok{  )}

\FunctionTok{names}\NormalTok{(sampvars) }\OtherTok{=} \FunctionTok{c}\NormalTok{(}\StringTok{"Error variance"}\NormalTok{,}\StringTok{"Between{-}net Variance"}\NormalTok{)}
...
1 0.6425266 0.4935532 0.1793128 0.5202356 1.723548 0.5202356    0.03776007
  mean.mc_std_err
1      0.01799568

$`Between-net Variance`
      mean        sd   q0.025     q0.5  q0.975   median sd.mc_std_err
1 2.131441 0.1847844 1.764094 2.111245 2.54298 2.111245   0.005025127
  mean.mc_std_err
1     0.006161214
...
Compute the mean, median, and quantiles for the ICC by drawing posterior
samples for \(\sigma^2_e\) and \(\sigma^2_u\) using the \texttt{predict}
function.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ICC }\OtherTok{\textless{}{-}}  \FunctionTok{predict}\NormalTok{(fit,PygmyWFBC, }\SpecialCharTok{\textasciitilde{}}\NormalTok{ \{}
\NormalTok{   tau\_e }\OtherTok{\textless{}{-}}\NormalTok{ Precision\_for\_the\_Gaussian\_observations}
\NormalTok{   tau\_u }\OtherTok{\textless{}{-}}\NormalTok{ Precision\_for\_net\_eff}
\NormalTok{   sigma\_u }\OtherTok{=} \DecValTok{1}\SpecialCharTok{/}\NormalTok{tau\_u}
\NormalTok{   sigma\_e }\OtherTok{=} \DecValTok{1}\SpecialCharTok{/}\NormalTok{tau\_e}
   \FunctionTok{list}\NormalTok{(}\AttributeTok{ICC =}\NormalTok{ sigma\_u}\SpecialCharTok{/}\NormalTok{ (sigma\_u}\SpecialCharTok{+}\NormalTok{sigma\_e))}
\NormalTok{   \},}
   \AttributeTok{n.samples =} \DecValTok{1000}
\NormalTok{  )}
\end{Highlighting}
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, leftrule=.75mm, rightrule=.15mm, opacityback=0, toprule=.15mm, coltitle=black, titlerule=0mm, left=2mm, colbacktitle=quarto-callout-note-color!10!white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Supplementary Material}, bottomtitle=1mm, toptitle=1mm, bottomrule=.15mm, colframe=quarto-callout-note-color-frame, opacitybacktitle=0.6, colback=white, breakable]
...
\begin{verbatim}
           sigma_e   sigma_u  
mean       2.124206  0.6509293
sd         0.1980611 0.4169733
quant0.025 1.763308  0.180072 
quant0.25  1.985235  0.3682855
quant0.5   2.113908  0.5419161
quant0.75  2.251968  0.8084173
quant0.975 2.540652  1.75623  
\end{verbatim}
