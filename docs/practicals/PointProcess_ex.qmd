...
In this case we will take a simple grid based approach where each quadrature location has an equal weight.
Our  grid consists of $N_k = 1000$ points and the weights are all  equal to $|\Omega|/N_k$. 
...
**OBS**: The implicit assumption here is that the intensity is constant inside each grid box, _and so is the covariate_!!
